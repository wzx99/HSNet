...
class RandomCropSegData(RandomCropData):
    def process(self, data):
        img = data['image']
        ori_img = img
        ori_lines = data['polys']
        ...
        for line in data['polys']:
            poly = ((np.array(line['points']) -
                     (crop_x, crop_y)) * scale).tolist()
            seg_poly = [((np.array(seg_point) -
                         (crop_x, crop_y)) * scale).tolist() for seg_point in line['seg_points']]
            if not self.is_poly_outside_rect(poly, 0, 0, w, h):
                lines.append({**line, 'points': poly, 'seg_points': seg_poly})
        data['polys'] = lines
