...
class Trainer:
    def __init__(self, experiment: Experiment):
        ...
        self.save_flag = experiment.main_flag
    ...
    def init_model(self):
        model = self.structure.builder.build(
            self.device, distributed=self.experiment.distributed, local_rank=self.experiment.local_rank)
        return model
    ...
    def train(self):
        ...
        while True:
            ...
            if self.experiment.distributed:
                self.experiment.train.data_loader.sampler.set_epoch(epoch)
    ...
    def train_step(self, model, optimizer, batch, epoch, step, **kwards):
        ...
        results = model.forward(batch, training=True, global_iter=step)
        if len(results) == 2:
            l, pred = results
            metrics = {}
        elif len(results) == 3:
        ...
        if isinstance(l, dict):
            ...
            for key, l_val in l.items():
                loss += l_val.mean()
                reduced_l_val = self.all_reduce_tensor(l_val.mean())
                line.append('loss_{0}:{1:.4f}'.format(key, reduced_l_val))
        else:
            loss = l.mean()
        loss.backward()
        optimizer.step()
        ...
        if step % self.experiment.logger.log_interval == 0:
            reduced_loss = self.all_reduce_tensor(loss)
            if isinstance(l, dict):
                line = '\t'.join(line)
                ...
                self.logger.info('step: %6d, epoch: %3d, loss: %.6f, lr: %f' % (
                    step, epoch, reduced_loss.item(), self.current_lr))
            self.logger.add_scalar('loss', reduced_loss, step)
            self.logger.add_scalar('learning_rate', self.current_lr, step)
            for name, metric in metrics.items():
                reduced_metric = self.all_reduce_tensor(metric.mean())
                self.logger.add_scalar(name, reduced_metric, step)
                self.logger.info('%s: %6f' % (name, reduced_metric))
    ...
    def all_reduce_tensor(self, tensor, norm=True):
        if self.experiment.distributed:
            ...
            if norm:
                tensor.div_(self.experiment.world_size)
            return tensor
        else:
