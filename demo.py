...
def main():
    ...
    parser.add_argument('--input_dir', type=str, help='image path')
    ...
    Demo(experiment, experiment_args, cmd=args).inference(args['input_dir'], args['visualize'])
...
class Demo:
    ...
    def inference(self, image_dir, visualize=False):
        ...
        for image in tqdm(images):
            ...
            with torch.no_grad():
                ...
                result_dir = os.path.join(self.args['result_dir'], filename)
                if not os.path.isdir(result_dir):
                    os.mkdir(result_dir)
                batch_boxes, batch_scores = output
                i=0
                for box, score in zip(batch_boxes[0], batch_scores[0]):
                    if score < self.args['box_thresh']:
                        continue
                    result_file_path = os.path.join(result_dir, str(i)+'.jpg')
                    box = np.array(box).reshape(-1)
                    x1,x2,y1,y2 = box[0::2].min(),box[0::2].max(),box[1::2].min(),box[1::2].max()
                    img_crop = ori_img[y1:y2,x1:x2]
                    cv2.imwrite(result_file_path, img_crop)  
                    i+=1
                    
                if visualize and self.structure.visualizer:
                    batch['ignore_tags'] = [False]*batch['image'].size(0)
