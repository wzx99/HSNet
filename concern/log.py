...
#     database_dir = State(default='./outputs/')
    ...
    def __init__(self, **kwargs):
        ...
        cmd = kwargs['cmd']
        if cmd.get('log_dir', None) is not None:
            self.log_dir = cmd['log_dir']
        ...
        self.log_flag = cmd.get('main_flag', True)
        
        if self.log_flag and not os.path.exists(self.log_dir):
...
#             os.symlink(storage_dir, self.log_dir)
    ...
    def _init_message_logger(self):
        ...
        if not message_logger.hasHandlers():
