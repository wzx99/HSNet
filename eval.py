...
def main():
    ...
    if args.get('resume', None) is not None and os.path.isdir(args['resume']):
        model_names = os.listdir(args['resume'])
        for model_name in model_names:
            new_args = args.copy()
            new_args.update({'resume':os.path.join(args['resume'], model_name)})
            Eval(experiment, experiment_args, cmd=new_args, verbose=args['verbose']).eval()
    else:
...
class Eval:
    def __init__(self, experiment, args, cmd=dict(), verbose=False):
        ...
        self.model_path = cmd.get(
            'resume', os.path.join(
                ...
                'final'))
        self.result_dir = cmd.get(
            'result_dir', self.logger.save_dir('result'))
    ...
    def init_model(self):
        self.structure.builder.model_args.update({'backbone_args':{'pretrained':False}})
        model = self.structure.builder.build(self.device, parallelized=False)
        return model
    ...
    def resume(self, model, path):
        ...
        if len(missing_keys_list) > 0:
            self.logger.warning('Missing key(s) in state_dict: {}'.format(
        ...
        if len(unexpected_keys_list) > 0:
            self.logger.warning('Unexpected key(s) in state_dict: {}'.format(
    ...
    def eval(self, visualize=False):
        ...
        with torch.no_grad():
            for _, data_loader in self.data_loaders.items():
                ...
                if self.args['test_speed']:
                    time_cost = np.array(time_costs).mean()
                    self.logger.info('Params: %s, Mean Inference speed: %fms, FPS: %f' % (
            str(sum(p.numel() for p in model.parameters() if p.requires_grad)),
            time_cost * 1000, 1 / time_cost))
                else:
                    metrics = self.structure.measurer.gather_measure(raw_metrics, self.logger)
                    self.logger.info('box_thresh : %f; thresh : %f' % (self.args['box_thresh'], self.args.get('thresh',0.3)))
