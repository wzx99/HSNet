...
from training.optimizer_scheduler import OptimizerScheduler, OptimizerScheduler2
...
class Experiment(Configurable):
    ...
    def __init__(self, **kwargs):
        ...
        self.world_size = cmd.get('num_gpus', 1)
        self.main_flag = (not self.distributed) or self.local_rank==0
        cmd['main_flag'] = self.main_flag
        kwargs.update(cmd=cmd)
        
        self.load_all(**kwargs)
        if cmd.get('train',True):
            self.load('train', **kwargs)
